...
const InternList: FC<{ interns: Intern[] }> = ({ interns }) => {
  const [sortedInterns, setSortedInterns] = useState<Intern[]>(interns);
  const [sortConfig, setSortConfig] = useState<{ key: keyof Intern; direction: 'asc' | 'desc' }>({
    key: 'name',
    direction: 'asc',
  });
  ...
  const handlePopoverToggle = (intern: Intern) => {
    ...
    <div className="overflow-x-auto bg-white shadow-lg rounded-lg">
      <Table className="min-w-full">
        <TableHeader className="bg-gray-100 text-gray-700 text-sm font-semibold">
          <TableRow>
            {['name', 'teamLead', 'role', 'email', 'performance', 'attendance', 'gitHubProjectLink', 'qualified', 'deploymentLink', 'pdfLink'].map((column) => (
              <TableHead
                key={column}
                className="cursor-pointer py-2 px-4"
                onClick={() => handleSort(column as keyof Intern)}
              >
        ...
        <TableBody>
          {sortedInterns.map((intern, index) => (
            <TableRow key={index} className="hover:bg-gray-50 transition-all duration-300">
              ...
              <TableCell className={`py-3 px-4 text-sm ${qualificationColors[String(intern.qualified)]}`}>
